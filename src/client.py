...
connection = pika.BlockingConnection(
        pika.ConnectionParameters(
            host=RABBITMQ_HOST,
            port=RABBITMQ_PORT, credentials=pika.PlainCredentials(username='admin', password='1234')
            )
        )
...
channel.queue_bind(exchange=RABBITMQ_EXCHANGE, queue=RABBITMQ_INBOUND_QUEUE_NAME, routing_key=RABBITMQ_INBOUND_ROUTING_KEY)
...
for i in range(100):
    ...
    channel.basic_publish(exchange=RABBITMQ_EXCHANGE, routing_key='task.schedule.create', body=json.dumps(data), properties=pika.BasicProperties(content_type='application/json'))
